<x-app-layout>
    <div class="bg-black text-white flex justify-center items-start">
        <div class="flex w-full items-center justify-center flex-col md:flex-row">
            ...
            <div class="w-full">
                @if ($media->type === 'image')
                    <img src="{{ $media->url }}" alt="Photo" class="w-full object-contain rounded h-[calc(60vh-32px)] md:h-[calc(100vh-64px)]">
                @elseif ($media->type === 'video')
                    <video src="{{ $media->url }}" controls class="w-full bg-black" style="max-height: calc(100vh - 64px);"></video>
                @endif
            </div>
